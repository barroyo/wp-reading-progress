...
function ruigehond006() {
    ...
    function initialize(p) {
        ...
        if (!document.getElementById('ruigehond006_bar')) {
            document.body.insertAdjacentHTML('beforeend',
                '<div id="ruigehond006_wrap" class="display: none;"><div id="ruigehond006_inner"><div id="ruigehond006_bar" role="progressbar" aria-valuemin="0" aria-valuemax="100" aria-valuenow="0" tabindex="-1"></div></div></div>');
    ...
    function progress(p) {
        ...
        //show the bar after a certain level of scroll
        if(window.scrollY > ruigehond006_c.start_at_scroll ) {
            document.getElementById('ruigehond006_wrap').style = "display: block;"
        } else {
            document.getElementById('ruigehond006_wrap').style = "display: none;"
        }
        // calculates de size of the content container and substract that one from the body, this will give us the size of the
        // elements below the content container
        // we will subsctract from the screensize calculations

        const textcontainer = document.getElementById(ruigehond006_c.content_container_id).getBoundingClientRect();
        const body = document.getElementsByTagName('body')[0].getBoundingClientRect();

        const exclude_footer_size = body.height - textcontainer.height;

        const loc_height = p.scrollHeight - heightCorrection - exclude_footer_size, // height in pixels = total amount that can be read
            reading_done = Math.max(Math.min(100 * (windowHeight - boundingClientTop(p) - heightCorrection - ruigehond006_c.start_at_scroll) / loc_height, 100), 0);
        requestAnimationFrame(function () {
            const el = document.getElementById('ruigehond006_bar');
